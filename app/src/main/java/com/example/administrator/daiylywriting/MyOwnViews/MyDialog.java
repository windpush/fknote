...
public class MyDialog {
    ...
    public Dialog webAddress(Context context, int style) {
        this.context = context;
        this.style = style;
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        LinearLayout dialogView = (LinearLayout) inflater.inflate(R.layout.dialog_webaddressedit, null);
        final Dialog customDialog = new Dialog(context, style);
    ...
    public Dialog deleteDialog(Context context, int style) {
        this.context = context;
        this.style = style;
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        CardView myPushDialogView = (CardView) inflater.inflate(R.layout.dialog_yesorno, null);
        final Dialog customDialog = new Dialog(context, style);
    ...
    public Dialog danMuDialog(Context context, int style,String color) {
        this.context = context;
        this.style = style;
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        CardView myPushDialogView = (CardView) inflater.inflate(R.layout.dialog_danmu, null);
        final Dialog customDialog = new Dialog(context, style);
        View colorShow =myPushDialogView.findViewById(R.id.colorShow);
        colorShow.setBackgroundColor(Color.parseColor(color));
        WindowManager.LayoutParams localLayoutParams = customDialog.getWindow().getAttributes();
        localLayoutParams.gravity = Gravity.CENTER | Gravity.CENTER;
        int screenWidth = ((Activity) context).getWindowManager().getDefaultDisplay().getWidth();
        myPushDialogView.setMinimumWidth(screenWidth*2/3);

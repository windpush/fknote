...
//        new Thread(this).start();
    ...
    public void initPieData(List<PieDateStruct> pieDateStructs){
        ...
        method=0;
        chart.setTitle("字数统计");
        chart.addSubtitle("(七天内)");
        this.invalidate();
        chartRender();
        new Thread(this).start();
    }

    public void initClickData(List<BookAddVaule> bookAddVaules){
        chartData.clear();
        int totalNumber =0;
        for (int i=0;i<bookAddVaules.size();i++){
            totalNumber=totalNumber+bookAddVaules.get(i).getAddClickNumber();
        }
        chartData.clear();
        String[] strings={"#00FFFF","#FF0000","#0000FF","#CC0099","#00FF00","#871F78","#FF7F00","#CC3300","#009933","#330066","#33CCFF","#6699FF","#FF66FF","#99CC66","#666666"};
        int total=0;
        for (int i=0;i<bookAddVaules.size();i++) {
            PieData pieData=null;
            if (i==(bookAddVaules.size()-1)){
                pieData = new PieData(bookAddVaules.get(i).getTimeVaule(), String.valueOf(bookAddVaules.get(i).getAddClickNumber()), 100-total, Color.parseColor(strings[i]));
            }else {
                 pieData = new PieData(bookAddVaules.get(i).getTimeVaule(), String.valueOf(bookAddVaules.get(i).getAddClickNumber()), bookAddVaules.get(i).getAddClickNumber() * 100 / totalNumber, Color.parseColor(strings[i]));
                total= total+bookAddVaules.get(i).getAddClickNumber() * 100 / totalNumber;
            }
            pieData.setCustLabelStyle(XEnum.SliceLabelStyle.INSIDE, Color.BLACK);
            chartData.add(pieData);
    ...
    public void initLikeData(List<BookAddVaule> bookAddVaules){
        chartData.clear();
        int totalNumber =0;
        for (int i=0;i<bookAddVaules.size();i++){
            totalNumber=totalNumber+bookAddVaules.get(i).getAddLikeNumber();
        }
        chartData.clear();
        String[] strings={"#00FFFF","#FF0000","#0000FF","#CC0099","#00FF00","#871F78","#FF7F00","#CC3300","#009933","#330066","#33CCFF","#6699FF","#FF66FF","#99CC66","#666666","#FF0000","#0000FF","#CC0099","#00FF00","#871F78","#FF7F00","#CC3300","#009933","#330066","#33CCFF","#6699FF","#FF66FF","#99CC66","#666666","#FF0000","#0000FF","#CC0099","#00FF00","#871F78","#FF7F00","#CC3300","#009933","#330066","#33CCFF","#6699FF","#FF66FF","#99CC66","#666666"};
        int total=0;
        for (int i=0;i<bookAddVaules.size();i++) {
            PieData pieData=null;
            if (i==(bookAddVaules.size()-1)){
                pieData = new PieData(bookAddVaules.get(i).getTimeVaule(), String.valueOf(bookAddVaules.get(i).getAddLikeNumber()), 100-total, Color.parseColor(strings[i]));
            }else {
                pieData = new PieData(bookAddVaules.get(i).getTimeVaule(), String.valueOf(bookAddVaules.get(i).getAddLikeNumber()), bookAddVaules.get(i).getAddLikeNumber() * 100 / totalNumber, Color.parseColor(strings[i]));
                total= total+bookAddVaules.get(i).getAddLikeNumber() * 100 / totalNumber;
            }
            pieData.setCustLabelStyle(XEnum.SliceLabelStyle.INSIDE, Color.BLACK);
            chartData.add(pieData);
...
//        chartData.add(pdTea);
    ...
    private void triggerClick(float x,float y)
    {
        ...
        if (method==0) {
            ...
            chart.getToolTip().addToolTip("你的书在" + pData.getKey() + "之间增加了"
                    + pData.getLabel() + "点击", mPaintToolTip);
        }else if (method==2){
            chart.getToolTip().addToolTip("你的书在" + pData.getKey() + "之间增加了"
                    + pData.getLabel() + "推荐", mPaintToolTip);
        }
        this.invalidate();
    }
